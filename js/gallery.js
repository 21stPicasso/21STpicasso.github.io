function modalFunc(image) {
  ...
  for (let i = 0; i < 9; i++) {
    const littleBox = document.createElement("img");
    littleBox.className = `box_${i} box`;
    if (i == 8) {
    } else {
      littleBox.src = `./images/resize_img/output/${image}_00${i + 1}.jpg`;
    }
  ...
  puzzleStart.onclick = function () {
    ...
    for (let i = 0; i < 9; i++) {
      const randBox = document.createElement("img");
      randBox.className = `box_${randArr[i]} box`;
      if (randArr[i] == 8) {
      } else {
        randBox.src = `./images/resize_img/output/${image}_00${
          randArr[i] + 1
        }.jpg`;
      }
      randBox.onclick = function () {
        ...
        eBox.src = this.src;
        this.src = "./images/resize_img/output/blank.png";
